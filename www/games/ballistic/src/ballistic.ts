...
export class Ballistic implements Drawable {
    ...
    constructor(public readonly width: number, public readonly height: number, loadedScores: Partial<HighScores>, private readonly saveData: (data: HighScores) => void) {
        ...
        this.help.push(new Page('Final Tips', 'The quicker you get the ball to\nthe goal, the more points you\nearn! Any lives remaining also\nincreases your score.\n\nYour *remaining* lives are shown\nat the top-right, so you actually\nhave one more life than it looks.\n\nKeep the rod mostly level for\nbetter control. Try to "feather"\neach side one-by-one to raise\nthe rod by tapping on the controls.\n\nThe harder the difficulty, the\nfaster the ball moves, but you\nearn a higher score multiplier.\n\nGood luck!', 'Press Space to continue or Escape to exit.'));
    }
